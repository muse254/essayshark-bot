pub mod types{
    ...
    impl FixedCache {
        ...
        pub fn id_is_present(&mut self, id: &String) -> bool {
        ...
        fn add_id(&mut self, id: String) {
            // cache is not full.
            if self.cache.contains(&String::from("")) {
                // find the first element with value = "" and update.
                if let Some((i, _)) = self.cache.iter().enumerate().find(|(_, id)| *id == "") {
                    self.cache[i] = id;
                    return;
                }
            }
            ...
            self.cache[self.overwrite_index] = id;
    
            // update the to the next index
            if self.overwrite_index == CACHE_SIZE - 1 {
